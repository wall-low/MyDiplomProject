...
- Push-уведомления
- Google Sign-In
- История платежей (отдельная страница)
...
**Формула:** `sum(rating * weight) / sum(weight)` за последние 180 дней

**Реализация:**
- `ReviewService._recalculateTutorRating()` — пересчёт после каждого отзыва
- `ReviewService._countPaidLessonsBetween()` — подсчёт веса
